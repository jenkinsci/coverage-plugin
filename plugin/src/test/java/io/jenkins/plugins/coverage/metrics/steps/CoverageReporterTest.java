...
class CoverageReporterTest {
    @Test
    void shouldAdjustReferenceBuildIfSelectedBuildHasNoAction() throws ReflectiveOperationException {
        ...
        try (MockedConstruction<ReferenceFinder> mockedReferenceFinder = mockConstruction(ReferenceFinder.class,
            ...
            assertThat(log.getInfoMessages()).contains("-> Reference build information adjusted");
            assertThat(mockedReferenceFinder.constructed()).hasSize(1);
        }
    }

    @Test
    void shouldHandleMissingPreviousBuildWhenSelectedReferenceHasNoAction() throws ReflectiveOperationException {
        Run<?, ?> currentBuild = createBuildWithAction("current-id", null);
        ...
        try (MockedConstruction<ReferenceFinder> mockedReferenceFinder = mockConstruction(ReferenceFinder.class,
            ...
            assertThat(log.getInfoMessages()).doesNotContain("-> Reference build information adjusted");
            assertThat(mockedReferenceFinder.constructed()).hasSize(1);
        }
    }
    ...
    private Optional<CoverageBuildAction> invokeGetReferenceBuildAction(
            final Run<?, ?> build, final String id, final FilteredLog log) throws ReflectiveOperationException {
        Method method = CoverageReporter.class.getDeclaredMethod(
                "getReferenceBuildAction", Run.class, String.class, FilteredLog.class);
        method.setAccessible(true);

        return (Optional<CoverageBuildAction>) method.invoke(new CoverageReporter(), build, id, log);
    }
}
