/* global $, proxy, echarts, CoverageColorScale */
...
(function ($) {
    ...
    // data-report-id attribute, set in index.jelly from ${it.id}) is used to remember the last selected metric,
    // independently for each report type (e.g. "coverage", "pit"), and the last configured green/red thresholds --
    // per metric, since the thresholds are only meaningful in the value range of one particular metric, except for
    // metrics that share the Overview tab's thresholds instead (see sharesOverviewThresholds below).
    ...
    // Metrics that visualize a 0-100% coverage-style value (see the "data-shares-overview-thresholds" attribute
    // set on the corresponding <option> in index.jelly, from TreeMapModel#sharesOverviewThresholds on the server)
    // share the same green/red thresholds as the Overview tab's gauge cards -- and the build-summary widget's
    // badges -- instead of their own, metric-specific thresholds, so a change made in any of those places stays
    // in sync everywhere it applies. All other metrics (e.g. Complexity, LOC) are not on that 0-100 scale and
    // keep their own per-metric, per-report thresholds, same as before.
    function sharesOverviewThresholds(option) {
        return !!option && option.getAttribute('data-shares-overview-thresholds') === 'true';
    }
    ...
    function persistThresholds(option, thresholds) {
        if (sharesOverviewThresholds(option)) {
            CoverageColorScale.persistThresholds(getReportId(), thresholds);
            return;
        }
        localStorage.setItem(thresholdStorageKey(option.value), JSON.stringify(thresholds));
    }

    function readPersistedThresholds(option) {
        if (sharesOverviewThresholds(option)) {
            return CoverageColorScale.readStoredThresholds(getReportId());
        }
        const stored = localStorage.getItem(thresholdStorageKey(option.value));
        if (!stored) {
            return null;
        }
        try {
    ...
    function loadThresholdsAndRender() {
        ...
        const persisted = readPersistedThresholds(option);
        if (persisted) {
        ...
        if (sharesOverviewThresholds(option)) {
            // No override stored yet (on the Overview tab or here) for this report -- fall back to the same
            // defaults the Overview tab uses, rather than an auto-detected value range, since that range would
            // not track "coverage" as a shared concept the way a fixed green/red pair does. Left unpersisted,
            // same as the Overview tab does until the user actually changes a threshold.
            const defaults = {green: CoverageColorScale.DEFAULT_GREEN, red: CoverageColorScale.DEFAULT_RED};
        ...
        proxy.getMetricValueRange(metricTagName, function (t) {
            ...
            applyThresholds(metricTagName, defaults);
            persistThresholds(option, defaults);
            renderChart();
        });
    }
    ...
    function onThresholdInputChanged() {
        ...
        persistThresholds(option, readThresholds());
        renderChart();
    }
